  ...
  <div class="min-h-screen bg-gradient-to-br from-pink-50 via-pink-100 to-pink-200 flex flex-col font-prompt overflow-x-auto">
    ...
    <li v-for="item in pagedProfits" :key="item.id" class="flex flex-col gap-2 sm:flex-row sm:justify-between sm:items-center border-b py-2">
      ...
      <div class="flex gap-2 flex-shrink-0 justify-end">
        <button class="bg-yellow-400 text-white px-3 xs:px-4 py-2 rounded-lg text-xs xs:text-sm font-bold shadow-sm hover:bg-yellow-500 transition-all" @click="editProfitFn(item.id)">แก้ไข</button>
        <button class="bg-red-500 text-white px-3 xs:px-4 py-2 rounded-lg text-xs xs:text-sm font-bold shadow-sm hover:bg-red-600 transition-all" @click="confirmDelete(item.id)">ลบ</button>
      </div>
    </li>
          </ul>
          <!-- Pagination -->
          ...
          <!-- Edit/Add Modal -->
          <div v-if="showEdit" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-2 xs:p-4">
            <div class="bg-white rounded-xl shadow-xl p-2 xs:p-4 sm:p-8 md:p-10 w-full max-w-full xs:max-w-md md:max-w-lg relative overflow-auto break-words max-h-screen overflow-y-auto">
              <button class="absolute top-2 right-2 text-gray-500 hover:text-red-500" @click="cancelEdit">✕</button>
              <h3 class="text-lg xs:text-xl font-bold mb-4 text-center">{{ editId === null ? 'เพิ่มรายการ' : 'แก้ไขรายการ' }}</h3>
              <div class="mb-3">
                <label class="block mb-1 font-bold">สินค้าในรายการ</label>
                <div class="hidden md:flex gap-2 mb-1 px-1 text-base font-semibold text-gray-600">
                  <div class="flex-1">ชื่อสินค้า</div>
                  <div class="w-16 text-center">จำนวน</div>
                  <div class="w-24 text-center">ราคาขาย</div>
                  <div class="w-24 text-center">ต้นทุน</div>
                  <div class="w-12"></div>
                </div>
                <div v-for="(prod, idx) in editItems" :key="idx" class="flex flex-col md:flex-row gap-2 mb-2 items-center bg-gray-50 border border-gray-200 rounded-lg p-2">
                  <div class="flex flex-col w-full md:hidden gap-1 mb-1">
                    <label class="text-xs font-semibold text-gray-600">ชื่อสินค้า</label>
                    <input v-model="prod.name" type="text" placeholder="ชื่อสินค้า" class="border rounded px-2 py-1 flex-1 min-w-[80px] text-base" />
                  </div>
                  <div class="flex flex-col w-full md:hidden gap-1 mb-1">
                    <label class="text-xs font-semibold text-gray-600">จำนวน</label>
                    <input v-model.number="prod.qty" type="number" min="1" placeholder="จำนวน" class="border rounded px-2 py-1 w-full text-center text-base" />
                  </div>
                  <div class="flex flex-col w-full md:hidden gap-1 mb-1">
                    <label class="text-xs font-semibold text-gray-600">ราคาขาย</label>
                    <input v-model.number="prod.price" type="number" min="0" placeholder="ราคาขาย" class="border rounded px-2 py-1 w-full text-center text-base" />
                  </div>
                  <div class="flex flex-col w-full md:hidden gap-1 mb-1">
                    <label class="text-xs font-semibold text-gray-600">ต้นทุน</label>
                    <input v-model.number="prod.cost" type="number" min="0" placeholder="ต้นทุน" class="border rounded px-2 py-1 w-full text-center text-base" />
                  </div>
                  <input v-model="prod.name" type="text" placeholder="ชื่อสินค้า" class="hidden md:block border rounded px-2 py-1 flex-1 min-w-[80px] text-base" />
                  <input v-model.number="prod.qty" type="number" min="1" placeholder="จำนวน" class="hidden md:block border rounded px-2 py-1 w-full sm:w-20 md:w-16 text-center text-base" />
                  <input v-model.number="prod.price" type="number" min="0" placeholder="ราคาขาย" class="hidden md:block border rounded px-2 py-1 w-full sm:w-28 md:w-24 text-center text-base" />
                  <input v-model.number="prod.cost" type="number" min="0" placeholder="ต้นทุน" class="hidden md:block border rounded px-2 py-1 w-full sm:w-28 md:w-24 text-center text-base" />
                  <button class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 transition-all w-full sm:w-20 md:w-12 mt-2 md:mt-0 text-base" @click="removeItem(idx)">ลบ</button>
                </div>
                <button class="bg-green-500 text-white px-3 py-1 rounded font-bold mt-1 w-full md:w-auto text-base" @click="addItem">+ เพิ่มสินค้า</button>
              </div>
              <div class="mb-3">
                <label class="block mb-1 font-bold">ประเภทการจ่าย</label>
                <select v-model="editPayType" class="border rounded px-2 py-1 w-full">
              ...
              <div class="mb-3">
                <label class="block mb-1 font-bold">วันเวลาขาย</label>
                <input v-model="editDatetime" type="datetime-local" class="border rounded px-2 py-1 w-full" />
              </div>
              <div class="flex flex-col xs:flex-row gap-2 mt-4">
                <button class="bg-pink-500 text-white px-3 xs:px-4 py-2 rounded font-bold flex-1" @click="saveEdit">บันทึก</button>
                <button class="bg-gray-300 text-gray-700 px-3 xs:px-4 py-2 rounded font-bold flex-1" @click="cancelEdit">ยกเลิก</button>
              </div>
            </div>
          </div>
          <!-- Delete Confirmation Modal -->
          <div v-if="showDeleteModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-1 xs:p-2">
            <div class="bg-white rounded-xl shadow-xl p-2 xs:p-3 sm:p-4 w-full max-w-xs relative overflow-auto break-words max-h-[90vh] overflow-y-auto">
              <button class="absolute top-2 right-2 text-gray-500 hover:text-red-500" @click="cancelDelete">✕</button>
              <h3 class="text-lg xs:text-xl font-bold mb-4 text-center">ยืนยันการลบรายการ</h3>
              <p class="mb-4 text-center text-gray-700">คุณต้องการลบรายการนี้ออกจากระบบหรือไม่?</p>
              <div class="flex flex-col xs:flex-row gap-2 mt-4 justify-center">
                <button class="bg-red-500 text-white px-3 xs:px-4 py-2 rounded font-bold flex-1" @click="doDelete">ลบ</button>
                <button class="bg-gray-300 text-gray-700 px-3 xs:px-4 py-2 rounded font-bold flex-1" @click="cancelDelete">ยกเลิก</button>
              </div>
            </div>
          </div>
        </div>
